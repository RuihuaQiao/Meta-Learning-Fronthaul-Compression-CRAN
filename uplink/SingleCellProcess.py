...
for nn in range(Nall):
    ### determine the serving RRH of UE nn
    bb = nn // Nc
    diag_M = torch.diag_embed(torch.ones((batch_size, M), dtype=torch.double).to(device))
    Hb = H_set[:, bb, :, :]
    hbn = H_set[:, bb, :, nn].view(batch_size, M, 1)

    ### local receive beamforming vector
    Cn = LA.inv(Hb @ Hb.mH + sigma2_zx_ratio * diag_M) @ hbn  # (bs, M, 1)
    Cn = Cn / LA.vector_norm(Cn, dim=-2, keepdims=True)  # (bs, M, 1)

    numerator = (torch.abs(hbn.mH @ Cn) ** 2).view(batch_size, 1)  # (bs, 1)
    power_all = 0
    for jj in range(Nall):
        hbj = H_set[:, bb, :, jj].view(batch_size, M, 1)  # (bs, M, 1)
        power_all = power_all + (torch.abs(hbj.mH @ Cn) ** 2).view(batch_size, 1)
    ...
    denominator = power_all - numerator + sigma2_zx_ratio * torch.ones((batch_size, 1), dtype=torch.double).to(device)  # (bs, 1)
