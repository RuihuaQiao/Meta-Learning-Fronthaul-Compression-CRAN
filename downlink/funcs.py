...
def gen_channel(UELocs, dist_set, M, scaling_factor):
    ...
    # Hc = torch.randn(bs, Ball, M, Nall).to(device)
...
def compute_Cn_Fnbar(H_set, W_set, Theta_set, sig2r):
    """
    compute transmit beamformer C(n) for all UEs
    ...
    for nn in range(Nall):
        ...
        Cn_set[nn] = Cn
    return Cn_set, Fn_bar_set


def expand_C_mtx(Theta_set, Cn_set):
    bs, Nall, B = Theta_set.shape
    K = int(Cn_set[0].shape[1]/B)
    Ball = 19
    C_mtx_redu = torch.cat([Cn_set[nn].mT for nn in range(Nall)], dim=1)  # (bs, Nall, BK)
    C_mtx_redu_4d = C_mtx_redu.view(bs, Nall, B, K)
    C_mtx_fullsize_4d_0 = torch.zeros((bs, Nall, Ball, K), device=device)
    Theta_set_4d = torch.cat(([Theta_set.unsqueeze(-1) for _ in range(K)]), dim=-1)
    C_mtx_fullsize_4d = torch.scatter(input=C_mtx_fullsize_4d_0, index=Theta_set_4d, src=C_mtx_redu_4d, dim=-2)
    C_mtx_fullsize = C_mtx_fullsize_4d.view(bs, Nall, Ball*K).mT  # (bs, Ball*K, Nall), has 0-entries

    return C_mtx_fullsize


def compute_rate_complete(H_set, W_set, Theta_set, Cn_set, Fn_bar_set, sig2r):
    """
    compute averaged sum rate (without removing the outlier samples)
    :param H_set: (bs, Ball, M, Nall)
    :param W_set: dict, Ball * (bs, K, M)
    :param Theta_set: (bs, Nall, B)
    :param Cn_set:
    ...
    Nc = Nall // Ball
    diag_BK = torch.diag_embed(torch.ones((bs, B * K)).to(device))
    # diag_pow_alloc = torch.diag_embed(pow_alloc)  # (bs, Nall, Nall)

    sum_rate = torch.zeros(bs, 1).to(device)
    stats_set = np.zeros((Nall, 2))

    for nn in range(Nall):
        fnn = Fn_bar_set[nn][..., nn].view(bs, B * K, 1)  # (bs, BK, 1)
        numerator = (torch.abs(fnn.mH @ Cn_set[nn]) ** 2).view(bs, 1)  # (bs, 1)
        # power_all = torch.sum(torch.abs(Fn_bar.mH @ Cn)**2, dim=1)  # (bs, 1)
        power_all = 0
        for jj in range(Nall):
            fnj = Fn_bar_set[jj][..., nn].view(bs, B * K, 1)  # (bs, BK, 1)
            power_all = power_all + (torch.abs(fnj.mH @ Cn_set[jj]) ** 2).view(bs, 1)
        ...
        denominator = interference + sig2r * torch.ones(bs, 1).to(device)  # (bs, 1)
    ...
    per_UE_rate = (sum_rate / Nall).squeeze()
    mean_per_UE_rate = torch.mean(per_UE_rate)  # average over samples in a batch

    return mean_per_UE_rate


def compute_rate(H_set, W_set, Theta_set, Cn_set, Fn_bar_set, sig2r):
    """
    ...
    # remove the worst samples for numerical stability (not used in testing phase)
    per_UE_rate_new = per_UE_rate[torch.sort(per_UE_rate)[1][10:]]
    mean_per_UE_rate = torch.mean(per_UE_rate_new)  # average over samples in a batch
...
def compute_rate_quant(H_set, W_set, Theta_set, C_mtx_fullsize, sig2r, quant_bits, gamma, if_test=1):
    """

    :param C_mtx_fullsize: (bs, Ball*K, Nall)
    :param H_set: (bs, Ball, M, Nall)
    :param W_set: dict, Ball * (bs, K, M)
    :param Theta_set: (bs, Nall, B)
    :param sig2r: simaz2 / sigmax2
    :param quant_bits: (bs, Ball, K), bits allocated for each dimension
    :param S_set: (bs, Ball, K), variance of each dimension
    ...
    for bb in range(Ball):
        Cb = C_mtx_fullsize[:, bb*K:(bb+1)*K, :]  # (bs, K, Nall)
        Sigma_vb = Cb @ Cb.mH  # (bs, K, K)
        # Fb = W_set[bb] @ H_set[:, bb, :, :]  # (bs, K, N)
        # Sigma_vb = Fb @ Fb.mH + sig2r * W_set[bb] @ W_set[bb].mH  # (bs, K, K)
        U, S, _ = LA.svd(Sigma_vb)  # S (bs, K), U (bs, K, K)
        S_set[:, bb, :] = S

    clip_range = gamma * torch.sqrt(S_set)  # (bs, Ball, K)
    delta_q = 2 * clip_range / (2 ** quant_bits)  # (bs, Ball, K)
    S_quant = delta_q ** 2 / 12  # (bs, Ball, K)
    S_quant_diag = torch.diag_embed(S_quant.view(bs, Ball*K))  # (bs, Ball*K, Ball*K)

    mask_set = torch.ones((bs, Ball, K), dtype=torch.bool).to(device)
    if if_test:
        x = torch.normal(0, 1, size=(bs, Nall, 1)).to(device)
        for bb in range(Ball):
            v_b = (C_mtx_fullsize[:, bb*K:(bb+1)*K, :] @ x).squeeze()  # (bs, K)
            ...
            err_v_b = v_b_Q - v_b
            mask_set[:, bb, :] = ~torch.gt(torch.abs(err_v_b.squeeze()), (delta_q_b / 2))  # entry=0 if out of clipping
    mask_set = mask_set.view(bs, Ball * K, 1)
    C_mtx_full_masked = C_mtx_fullsize * mask_set.expand(bs, Ball*K, Nall)  # (bs, Ball*K, Nall)
    C_mtx_redu_masked = torch.gather(input=C_mtx_full_masked.mT.view(bs, Nall, Ball, K),
                                     index=torch.cat(([Theta_set.unsqueeze(-1) for _ in range(K)]), dim=-1),
                                     dim=-2).view(bs, Nall, B*K).mT  # (bs, B*K, Nall)
    Cn_set = {nn: C_mtx_redu_masked[..., nn].unsqueeze(-1) for nn in range(Nall)}  # Cn after mask

    sum_rate = torch.zeros(bs, 1).to(device)
    stats_set = np.zeros((Nall, 3))

    H_all = torch.cat([H_set[:, bb, :, :] for bb in range(Ball)], dim=1)  # (bs, Ball*M, Nall)
    W_all = torch.zeros((bs, Ball * K, Ball * M)).to(device)
    for bb in range(Ball):
        W_all[:, (bb * K):((bb + 1) * K), (bb * M):((bb + 1) * M)] = W_set[bb]  # (bs, K, M)
    F_all = W_all @ H_all  # (bs, Ball*K, Nall)

    Fn_bar_set = {}  # need to recompute Fn_bar_tilde according to W_tilde
    for nn in range(Nall):
        ...
        Fn_bar_set[nn] = Wn_bar @ Hn_bar  # (bs, BK, Nall)
    ...
    for nn in range(Nall):
        ...
        f_n = F_all[..., nn].unsqueeze(-1)  # (bs, Ball*K, 1)
        if if_test:
            f_n = f_n * mask_set
        pow_quant = (f_n.mH @ S_quant_diag @ f_n).view(bs, 1)  # (bs, 1)

        denominator = interference + sig2r * torch.ones(bs, 1).to(device) + pow_quant   # (bs, 1)
        Rn = torch.log(1 + numerator / denominator / db2pow(6)) / np.log(2)
        sum_rate = sum_rate + Rn  # (bs, 1)
