...
class GRU_meta(nn.Module):
    ...
    def forward(self, W_set, H_set, Theta_set):
        ...
        Cn_set, Fn_bar_set = compute_Cn_Fnbar(H_set, W_set, Theta_set, sigma2_zx_ratio)
        if W_set[0].requires_grad:  # training stage
            R_ini = compute_rate(H_set, W_set, Theta_set, Cn_set, Fn_bar_set, sigma2_zx_ratio)
        else:
            R_ini = compute_rate_complete(H_set, W_set, Theta_set, Cn_set, Fn_bar_set, sigma2_zx_ratio)
        R_sum_time = 0  # tensor with grad info
        R_list = [R_ini.item()]  # list without grad info
        ...
        for eee in range(time_step):
            ...
            # gamma_np = np.array(3)
            W_grad = autograd(W_np, H_set, Theta_set, sigma2_zx_ratio)
            ...
            Cn_set, Fn_bar_set = compute_Cn_Fnbar(H_set, W_set, Theta_set, sigma2_zx_ratio)
            if W_set[0].requires_grad:  # training stage
                R_new = compute_rate(H_set, W_set, Theta_set,  Cn_set, Fn_bar_set, sigma2_zx_ratio)
            else:
                R_new = compute_rate_complete(H_set, W_set, Theta_set,  Cn_set, Fn_bar_set, sigma2_zx_ratio)
            R_sum_time = R_sum_time + (time_step - eee - 1) * R_new
            R_list.append(R_new.item())

        R_avg_time = R_sum_time / np.sum(np.arange(time_step))

        return W_set, R_avg_time, R_list


### model path
model_path = './saved_model/UserCentric_DL_GRU_cell{}_Nc{}_M{}_B{}_K{}_step{}'.format(Ball, Nc, M, B, K, time_step_load)
...
if initial_run == 0:  # continue to train
    gru_meta.load_state_dict(torch.load(model_path+"/GRU_ul_all0_dl_new.zip", map_location=torch.device(device)))
...
##### training
torch.manual_seed(1)
# params = list(gru_meta.parameters()) + list(DNN_local.parameters())
params = gru_meta.parameters()
...
with torch.no_grad():
    ...
    _, _, rate_val_list = gru_meta(W_set_val, H_set_val, Theta_set_val)
...
for ee in range(num_epochs):
    ...
    for ii in range(batch_per_epoch):
        ...
        _, rate_train_avg, rate_train_list = gru_meta(W_set_train, H_set_train, Theta_set_train)
    ...
    if (ee+1) % 5 == 0 and rate_val_list[-1] > best_rate_val:
        ...
        torch.save(gru_meta.state_dict(), model_path+'/GRU_ul_all0_dl_new.zip')
        # savemat(model_path + "/hidden_state.mat", {"h": ini_h.cpu().detach().numpy()})
...
with torch.no_grad():
    ...
    _, _, rate_test_list = gru_meta(W_set_test, H_set_test, Theta_set_test)
    ...
    savemat("./plot_result/plot_convergence/GRU_M{}_B{}_K{}_step{}.mat".format(M, B, K, time_step), result_dict)
